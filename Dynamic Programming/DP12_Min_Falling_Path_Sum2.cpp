...
#include <bits/stdc++.h>
using namespace std;
int f(int i, int prev_task, int n, vector<vector<int>> &matrix, vector<vector<int>> &dp)
{
    ...
    if(dp[i][prev_task]!=-1)return dp[i][prev_task];
    int mini = 1e9;
    for (int task = 0; task < n; task++)
    {
        if (task != prev_task)
        {
            int store = matrix[i][task] + f(i - 1,task,n, matrix, dp);
            mini = min(store, mini);
        }
    }
...
int minFallingPathSum( vector<vector<int>>&matrix)
{
    ...
    // for(int i=1;i<n;i++)
    // {
    //     for(int prev_task=n-1;prev_task>=0;prev_task--)
    //     {
    //         int mini = 1e9;
    //         for (int task = 0; task < n; task++)
    //         {
